...
@click.command()
@click.option('--info', 'i', required=True)
def describe(info):
    click.echo(plot_description)
@click.option('--graph', '-g', type=click.Choice(graph_numbers))
@click.option('--year', '-y')
def plotting(graph):
    my_dict = {'1': wind_1, '2': wind_2, '3': wind_3}
